...
bool SemiGlobalMatching::Initialize(const std::int32_t& height, const std::int32_t& width, const SGMOption& option) {
	// 影像尺寸
    height_ = height;
    width_ = width;
    // SGM参数
    ...
    // census值（左右影像）
    const std::int32_t img_size = width * height;
    if (option.census_size == Census5x5) {
        left_census_ = new std::uint32_t[img_size]();
        right_census_ = new std::uint32_t[img_size]();
    }
    else {
        left_census_ = new std::uint64_t[img_size]();
        right_census_ = new std::uint64_t[img_size]();
    }

    // 视差范围
    const std::int32_t disp_range = option.max_disparity - option.min_disparity;
    if (disp_range <= 0) {
        return false;
    }

    // 匹配代价（初始/聚合）
    const std::int32_t size = width * height * disp_range;
    cost_init_   = new std::uint8_t[size]();
    cost_aggr_   = new std::uint16_t[size]();
    cost_aggr_1_ = new std::uint8_t[size]();
    cost_aggr_2_ = new std::uint8_t[size]();
    cost_aggr_3_ = new std::uint8_t[size]();
    cost_aggr_4_ = new std::uint8_t[size]();
    cost_aggr_5_ = new std::uint8_t[size]();
    cost_aggr_6_ = new std::uint8_t[size]();
    cost_aggr_7_ = new std::uint8_t[size]();
    cost_aggr_8_ = new std::uint8_t[size]();

    // 视差图
    left_disp_ = new float[img_size]();
    right_disp_ = new float[img_size]();
...
//        sgm_util::RemoveSpeckles(left_disp_, width_, height_, 1, option_.min_speckle_aera, Invalid_Float);
//    }
//
//    // 视差填充
...
//
//    // 中值滤波
//    sgm_util::MedianFilter(left_disp_, left_disp_, width_, height_, 3);
//
//    end = steady_clock::now();
//    tt = duration_cast<milliseconds>(end - start);
...
//        sgm_util::census_transform_5x5(left_image_, static_cast<std::uint32_t*>(left_census_), width_, height_);
//        sgm_util::census_transform_5x5(right_image_, static_cast<std::uint32_t*>(right_census_), width_, height_);
//    }else {
//        sgm_util::census_transform_9x7(left_image_, static_cast<uint64*>(left_census_), width_, height_);
//        sgm_util::census_transform_9x7(right_image_, static_cast<uint64*>(right_census_), width_, height_);
//    }
//}
//
//void SemiGlobalMatching::ComputeCost() const {
//    const std::int32_t& min_disparity = option_.min_disparity;
//    const std::int32_t& max_disparity = option_.max_disparity;
...
//            // 逐视差计算代价值
//        	for (std::int32_t d = min_disparity; d < max_disparity; d++) {
...
//                if (option_.census_size == Census5x5) {
//                    // 左影像census值
//                    const auto& census_val_l = static_cast<std::uint32_t*>(left_census_)[i * width_ + j];
//                    // 右影像对应像点的census值
//                    const auto& census_val_r = static_cast<std::uint32_t*>(right_census_)[i * width_ + j - d];
//                    // 计算匹配代价
//                    cost = sgm_util::Hamming32(census_val_l, census_val_r);
//                }
//        		else {
//                    const auto& census_val_l = static_cast<uint64*>(left_census_)[i * width_ + j];
//                    const auto& census_val_r = static_cast<uint64*>(right_census_)[i * width_ + j - d];
...
//
//    const std::int32_t size = width_ * height_ * (max_disparity - min_disparity);
//    if(size <= 0) {
//        return;
//    }
//
...
//        // 左右聚合
//        sgm_util::CostAggregateLeftRight(left_image_, width_, height_, min_disparity, max_disparity, P1, P2_Int, cost_init_, cost_aggr_1_, true);
//        sgm_util::CostAggregateLeftRight(left_image_, width_, height_, min_disparity, max_disparity, P1, P2_Int, cost_init_, cost_aggr_2_, false);
//        // 上下聚合
//		sgm_util::CostAggregateUpDown(left_image_, width_, height_, min_disparity, max_disparity, P1, P2_Int, cost_init_, cost_aggr_3_, true);
//        sgm_util::CostAggregateUpDown(left_image_, width_, height_, min_disparity, max_disparity, P1, P2_Int, cost_init_, cost_aggr_4_, false);
//    }
//
//    if (option_.num_paths == 8) {
//        // 对角线1聚合
//        sgm_util::CostAggregateDagonal_1(left_image_, width_, height_, min_disparity, max_disparity, P1, P2_Int, cost_init_, cost_aggr_5_, true);
//        sgm_util::CostAggregateDagonal_1(left_image_, width_, height_, min_disparity, max_disparity, P1, P2_Int, cost_init_, cost_aggr_6_, false);
//        // 对角线2聚合
//        sgm_util::CostAggregateDagonal_2(left_image_, width_, height_, min_disparity, max_disparity, P1, P2_Int, cost_init_, cost_aggr_7_, true);
//        sgm_util::CostAggregateDagonal_2(left_image_, width_, height_, min_disparity, max_disparity, P1, P2_Int, cost_init_, cost_aggr_8_, false);
//    }
//
//    // 把4/8个方向加起来
...
//
//    // 左影像视差图
//    const auto disparity = left_disp_;
//	// 左影像聚合代价数组
//	const auto cost_ptr = cost_aggr_;
//
//    const std::int32_t width = width_;
//    const std::int32_t height = height_;
...
//            std::int32_t best_disparity = 0;
//
//            // ---遍历视差范围内的所有代价值，输出最小代价值及对应的视差值
//            for (std::int32_t d = min_disparity; d < max_disparity; d++) {
//	            const std::int32_t d_idx = d - min_disparity;
...
//            const std::int32_t idx_1 = best_disparity - 1 - min_disparity;
//            const std::int32_t idx_2 = best_disparity + 1 - min_disparity;
...
//
//    // 右影像视差图
//    const auto disparity = right_disp_;
//    // 左影像聚合代价数组
//	const auto cost_ptr = cost_aggr_;
//
//    const std::int32_t width = width_;
...
//        	for (std::int32_t d = min_disparity; d < max_disparity; d++) {
//                const std::int32_t d_idx = d - min_disparity;
//        		const std::int32_t col_left = j + d;
...
//
//            // 根据视差值找到右影像上对应的同名像素
//        	const auto col_right = static_cast<std::int32_t>(j - disp + 0.5);
//            
...
//					// else 
//        			//		pixel in mismatches
//					const std::int32_t col_rl = static_cast<std::int32_t>(col_right + disp_r + 0.5);
...
//	float *angle = angle1;
//    // 最大搜索行程，没有必要搜索过远的像素
//    const std::int32_t max_search_length = 1.0*std::max(abs(option_.max_disparity), abs(option_.min_disparity));
//
...
//		std::vector<std::pair<std::int32_t, std::int32_t>> inv_pixels;
//		if (k == 2) {
//			//  第三次循环处理前两次没有处理干净的像素
//			for (std::int32_t i = 0; i < height; i++) {
//				for (std::int32_t j = 0; j < width; j++) {
...
//            const std::int32_t y = pix.first;
//            const std::int32_t x = pix.second;
//
//			if (y == height / 2) {
//				angle = angle2; 
//			}
...
//				for (std::int32_t m = 1; m < max_search_length; m++) {
//					const std::int32_t yy = lround(y + m * sina);
//					const std::int32_t xx = lround(x + m * cosa);
