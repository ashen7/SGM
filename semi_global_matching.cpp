...
bool SemiGlobalMatching::Match(const std::uint8_t* left_image, const std::uint8_t* right_image, float* left_disp) {
    ...
    LOG(INFO) << "computing cost! timing : " << cost_time.count() / 1000.0 << "s";
    ...
    LOG(INFO) << "cost aggregating! timing : " << cost_time.count() / 1000.0 << "s";
    ...
    cost_time = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
    LOG(INFO) << "computing disparities! timing : " << cost_time.count() / 1000.0 << "s";
    ...
    cost_time = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
    LOG(INFO) << "postprocessing! timing : " << cost_time.count() / 1000.0 << "s";
        ...
        for (int j = 0; j < width_; j++) {
        	...
        	for (int d = min_disparity; d < max_disparity; d++) {
                ...
                if (option_.census_size == Census5x5) {
                    ...
                    const auto& census_val_l = static_cast<std::uint64_t*>(left_census_)[i * width_ + j];
                    const auto& census_val_r = static_cast<std::uint64_t*>(right_census_)[i * width_ + j - d];
